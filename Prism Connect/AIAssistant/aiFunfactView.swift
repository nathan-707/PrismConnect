...
struct aiFunfactView: View {
    ...
    var body: some View {
        VStack {
            ...
            else if aiIsEnabled && aiLightControlAssistant?.useChatGPTForFunfact == false {
                if let funFact = aiLightControlAssistant?.funfactResponse?.funfact {
                    ScrollView {
                        Text(funFact)
                            .font(.headline)
                            .padding(.top)
            ...
            if aiLightControlAssistant?.session.isResponding == false && aiLightControlAssistant?.useChatGPTForFunfact == false
//                || chatGPTsuccess && aiLightControlAssistant?.useChatGPTForFunfact == true
        ...
        .task {
            ...
            if onlyGiveFactFromLocal {
                ...
                aiIsEnabled = setupAI()
                aiLightControlAssistant = AIAssistant(prismSessionManager: prismSessionManager)
                aiLightControlAssistant?.prewarm()
            
                Task {
                    
                    if aiLightControlAssistant?.useChatGPTForFunfact == true { // chatgpt
    ...
    func getRandomFactFromLocal() -> String {
        if prismSessionManager.currentMode == .teleportMode {
            return prismSessionManager.CurrentTeleportation.funfacts.randomElement()!
        }
        
        else {
            return prismSessionManager.CurrentParkClockIsIn.funfacts.randomElement()!
        }
    }
    ...
    func getChatGPTFunfact() async throws {
        ...
        if prismSessionManager.currentMode == .themeParkMode  {
            ...
            funfact = prismSessionManager.CurrentTeleportation.nameForPicker()
        }
        
         funfact = try await (aiLightControlAssistant?.chatGPTFunfact(location: funfact))!
    ...
    func requestFunfact() async throws {
        ...
        do {
            ...
            try await aiLightControlAssistant?.getTeleportFunfact(cityIndex: prismSessionManager.CurrentTeleportation)
