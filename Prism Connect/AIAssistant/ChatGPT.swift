...
//    var model: String = "gpt-4o-mini"
    ...
    func sendMessage(_ message: String) async throws -> String {
        guard let apiKey, !apiKey.isEmpty else { throw ChatGPTError.missingAPIKey }
        ...
        if let firstChoice = decoded.choices.first {
            if let content = firstChoice.message.content, content.isEmpty == false {
                return content
            } else if let contentArray = firstChoice.message.multiContent, !contentArray.isEmpty {
                // Join any text parts if present
                let textJoined = contentArray.compactMap { $0.text }.joined(separator: " ")
...
struct AssistantMessage: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if let arrayContent = try? container.decode([ContentPart].self, forKey: .content) {
            content = nil
