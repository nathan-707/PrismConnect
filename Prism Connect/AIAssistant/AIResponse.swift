...
final class AIAssistant {
    ...
    let funfactInstructions =
        "You are a tour guide that gives 100 percent true facts about the given locations. Use the database with the getfunfact tool to create a funfact based on the database."
    ...
    func chatGPTFunfact(location: String) async throws -> String {
        let response = try await ChatGPTClient.shared.sendMessage(
            "Give me a truely random funfact for the given location. this may be called a lot and i want a different fact everytime.  \(location)"
        )
        return response
    }
    ...
    func getTeleportFunfact(cityIndex: City) async throws {
        ...
        self.session = LanguageModelSession(
            model: myModel,
            tools: [GetFunfactBasedOffInformationTool()],
            instructions: funfactInstructions
        )

        let stream = session.streamResponse(
            generating: AIFunfact.self,
            includeSchemaInPrompt: true,
            options: funfactOptions,
            prompt: {
                "Use the given tool to learn about \(Prompt(cityIndex.city)) then give a funfact based on what you learn from the on board database through the tool."
            }
        )
