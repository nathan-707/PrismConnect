...
struct EffectSpeedPickerView: View {
    ...
    var body: some View {
        ...
        if prismSessionManager.currentLightEffect == .dualmode_m {
            HStack {
                VStack{
                    ...
                    HStack {
                        ...
                        Picker("largeMode", selection: $prismSessionManager.largeMode) {
                    ...
                    HStack{
                        ...
                        Picker("smallMode", selection: $prismSessionManager.smallMode) {
                            Text("Custom Color").tag(6)
                            Text("Spectrum").tag(1)
                            Text("Headless").tag(2)
                            Text("Short Circuit").tag(3)
            ...
            .onChange(of: prismSessionManager.smallMode) { oldValue, newValue in
                notPendingImpact.impactOccurred()
                prismSessionManager.updateSettings(nameOfSetting: "smallMode", value: newValue)
            }
            .onChange(of: prismSessionManager.largeMode) { oldValue, newValue in
                notPendingImpact.impactOccurred()
                prismSessionManager.updateSettings(nameOfSetting: "largeMode", value: newValue)
            }
        }
        
        else if prismSessionManager.currentLightEffect == .rainbowmode_m {
            Picker("", selection: $prismSessionManager.SpecFS, content: {
            ...
            Picker("", selection: $prismSessionManager.HeadFS, content: {
                Text("Fast").tag(Float(0.0))
            ...
            Picker("", selection: $prismSessionManager.SCFS, content: {
                Text("Fast").tag(Float(0.0))
            ...
            Picker("", selection: $prismSessionManager.FireFS, content: {
                Text("Fast").tag(Float(0.0))
