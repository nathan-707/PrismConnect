        ...
        Form {
            Section(header: Text("Brightness")) {
                ...
                if !prismSessionManager.autoBrightnessOn {
                    Slider(value: $prismSessionManager.brightness, in: 0 ... 1)
                        ...
                        .onChange(of: prismSessionManager.brightness) { oldValue, newValue in
                            ...
                            if newValue > stepWatch + 0.1 {
                                ...
                                if newValue > 0.90 {
                                    prismSessionManager.updateSettings(nameOfSetting: "brightness", value: 1.0)
                                } else {
                                ...
                                if newValue < 0.1 {
                                    prismSessionManager.updateSettings(nameOfSetting: "brightness", value: 0)
                                }
                                else {
                                    prismSessionManager.updateSettings(nameOfSetting: "brightness", value: newValue)
                                }
            ...
            Section(header: Text("When Clock Turns On/Off")){
                Toggle("When Room is Dark (Automatic)", isOn: $prismSessionManager.autoOff)
                ...
                if !prismSessionManager.autoOff {
                    OnTimePicker()
                    OffTimePicker()
                    Toggle("Wait Until Room is Dark After Off Time", isOn: $prismSessionManager.semiAutoTurnOff)
            ...
            Section(header: Text(prismSessionManager.currentMode == .teleportMode ? "Exit Teleport Mode to Edit" : "Display Layout")) {
                LayoutPicker()
            }
            .disabled(prismSessionManager.currentMode == .teleportMode ? true : false)
            
            Section(header: Text("Tele-Settings")){
                TourIntervalPicker()
                
                Toggle("Show Teleporting Location Time", isOn: $prismSessionManager.getTimeInTel)
                    .onChange(of: prismSessionManager.getTimeInTel) { oldValue, newValue in
                        prismSessionManager.updateSettings(nameOfSetting: "getTimeInTel", value: (newValue == false ? 0 : 1))
                        // john
            ...
            Section(header: Text("Weather Alerts")) {
                Toggle("Hide Alerts", isOn: $prismSessionManager.ignoreAlerts)
                    .onChange(of: prismSessionManager.ignoreAlerts) { oldValue, newValue in
                        
                        prismSessionManager.updateSettings(nameOfSetting: "ignoreAlerts", value: (newValue == false ? 0 : 1))
                    }
                
                Toggle("Disable Alert Flashing", isOn: $prismSessionManager.disableAlertFlashing)
                    .onChange(of: prismSessionManager.disableAlertFlashing) { oldValue, newValue in
                        prismSessionManager.updateSettings(nameOfSetting: "disaAB", value: (newValue == false ? 0 : 1))
                    }
            }
...
//        }
        ...
        Stepper(String(prismSessionManager.currentLayout), value: $prismSessionManager.currentLayout, in: 1 ... 12)
        ...
        .onChange(of: prismSessionManager.cutOff, { oldValue, newValue in
