...
enum GetWeatherInterval {
    case normal, short, failing
...
class VirtualColorClock: NSObject, CLLocationManagerDelegate {
    ...
    static private let homeWeatherRefreshMins: Double = 30
    private var homeWeatherRefreshInterval: TimeInterval = homeWeatherRefreshMins * 60
    ...
    func getWeather(mode: Modes, city: City) async -> Bool {
        ...
        if mode != .home {
            ...
            return await fetchWeather(using: cityLocation, mode: mode, teleportCity: city)
        }
        ...
        switch status {
        case .notDetermined:
            print("Location auth not determined. Requesting When-In-Use authorization…")
            locationManager.requestWhenInUseAuthorization()
            return false
            
        case .authorizedWhenInUse, .authorizedAlways:
            if gotLocation {
                return await fetchWeather(using: homeLocation, mode: mode, teleportCity: city)
            } else {
        ...
        case .denied, .restricted:
            print("Location access denied or restricted; cannot fetch home weather.")
            Task{ @MainActor in
    ...
    @MainActor
    private func fetchWeather(using location: CLLocation, mode: Modes, teleportCity: City) async -> Bool {
        do {
            ...
            if mode == .home {
                if let last = lastFetchDate {
                    ...
                    if elapsed < homeWeatherRefreshInterval {
                        print("Skipping weather fetch; only \(Int(elapsed))s elapsed (< \(Int(homeWeatherRefreshInterval))s interval)")
                        ...
                        currentWeather.temp_feelsLike = homeLocationCache.temp_feelsLike
                        currentWeather.weatherDescription = homeLocationCache.weatherDescription
                        currentWeather.weatherMain = homeLocationCache.weatherMain
                        currentWeather.weatherLight = homeLocationCache.weatherLight
                        return true
                    }
                }
            }
                ...
                if let lastTeleportCityDate = teleportCity.lastWeatherFetch {
                    ...
                    if elapsed < getTeleportFetchCoolOffInterval {
                        print("Skipping weather fetch for \(teleportCity.city); only \(Int(elapsed))s elapsed (< \(Int(getTeleportFetchCoolOffInterval))s interval)")
                        ...
                        currentWeather.temp_feelsLike = weatherCache.temp_feelsLike
                        currentWeather.weatherDescription = weatherCache.weatherDescription
            ...
            // teleporting if here.
            print("attempting weather for \(location.coordinate.latitude) \(location.coordinate.longitude)")
            let service = WeatherService.shared
            let weather = try await service.weather(for: location)
            self.currentWeather.isDayLight = weather.currentWeather.isDaylight
            self.currentWeather.intensity = weather.currentWeather.precipitationIntensity.value
            self.currentWeather.temp_main = weather.currentWeather.temperature.converted(to: .fahrenheit).value
            self.currentWeather.temp_feelsLike = weather.currentWeather.apparentTemperature.converted(to: .fahrenheit).value
            self.currentWeather.weatherDescription = weather.currentWeather.condition.description
            ...
            if mode == .home { // update home weather cache and interval
                ...
                homeLocationCache.weatherDescription = currentWeather.weatherDescription
                ...
                if let idx = ALL_CITIES.firstIndex(where: { $0.nameForPicker() == teleportCity.nameForPicker() }) {
                    ALL_CITIES[idx].lastWeatherFetch = Date()
                    ALL_CITIES[idx].weatherCache.isDayLight = currentWeather.isDayLight
                    ALL_CITIES[idx].weatherCache.intensity = currentWeather.intensity
                    ALL_CITIES[idx].weatherCache.temp_main = currentWeather.temp_main
                    ALL_CITIES[idx].weatherCache.temp_feelsLike = currentWeather.temp_feelsLike
                    ALL_CITIES[idx].weatherCache.weatherDescription = currentWeather.weatherDescription
                    ALL_CITIES[idx].weatherCache.weatherMain = currentWeather.weatherMain
                    ALL_CITIES[idx].weatherCache.weatherLight = currentWeather.weatherLight
                }
            }
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        gotLocation = true
        homeLocation = CLLocation(latitude: location.coordinate.latitude, longitude: location.coordinate.longitude)
        print("Got home location: \(homeLocation.coordinate.latitude), \(homeLocation.coordinate.longitude)")
    }
